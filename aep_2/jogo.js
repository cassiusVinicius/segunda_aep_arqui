...
let dx = 2
let dy = -2
let upgrade = 1// criada a variavel upgrade
let pontos = 0
let vidas = 5
    ...
    for (let l = 0; l < contagemDeBlocosEmLinha; l++) {
      ...
      if (bloco.estado === 1) {
        if (
          ...
          if (pontos == (contagemDeBlocosEmLinha * contagemDeBlocosEmColuna) * upgrade) {//colocado entre parenteses
            alert('Você completou o desafio e passará para outro upgrade')
            upgrade++// adicionando o contador

            for (let c = 0; c < contagemDeBlocosEmColuna; c++) {//laço adicionado
              blocos[c] = []
              for (let l = 0; l < contagemDeBlocosEmLinha; l++) {
                blocos[c][l] = { x: 0, y: 0, estado: 1 }
              }
            ...
            x = canvas.width / 2
            y = canvas.height - 30
            dx = 2 + upgrade // mudança 
            dy = -2 - upgrade
            raqueteX = (canvas.width - larguraDaRaquete) / 2
            desenha()//chamando a função que reconstroí os blocos
          }
        }
      }
    }
